(function () {
  ...
  function buildToc(container) {
    ...
    heads.forEach(h => {
      if (!h.id) {
        h.id = h.textContent.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^
w\-ąćęłńóśżź]/gi, '');
      }
      const a = document.createElement('a');
      a.href = `#${h.id}`;
  ...
  async function selectModule(id) {
    ...
    try {
      ...
      console.error(e);
      elContent.innerHTML = `<div class="loading">⚠️ Nie udało się wczytać modułu. Sprawdź ścieżkę: <code>${mod.localPath}</code></div>`;
    }
  }
