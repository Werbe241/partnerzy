const CERT_DEFAULT = {
  ...
  certNo: genCertNo("KR"),
  verifyBase: location.origin + "/kk/weryfikacja/?cert_no="
};
...
function init(){
  ...
  const no = CERT_DATA.certNo || genCertNo(role);
  ...
  const verifyUrl = (CERT_DATA.verifyBase || CERT_DEFAULT.verifyBase) + encodeURIComponent(no);
